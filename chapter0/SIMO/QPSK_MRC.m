...
for snr_num=1:length(ebnodb)
    ...
    %QPSK信号生成
    data1=rand(1,nd)>0.5;
    data2=qpsk_modulation(data1);
    data2=data2./sqrt(2); %能量归一化
    
    %模拟 1Tx 2Rx
    sigma=calNoisePower(4,data2,ebnodb(snr_num));
    %第1径
    ...
    % QPSK 解调 for MRC
    [demodata1,uu]=qpsk_demodulation(data4);
    noe2=sum(abs(data1-demodata1));
    nod2=length(data1);
    noe=noe+noe2;
    nod=nod+nod2;
    % QPSK 解调 for EGC
    [demodata1egc,uu]=qpsk_demodulation(data4egc);
    ...
    theory_rayleigh(snr_num) = berfading(ebnodb(snr_num),'psk',4,2);
end 
