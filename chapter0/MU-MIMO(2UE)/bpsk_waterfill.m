...
for ii = 1:length(Eb_N0_dB)
   ...
   ip = rand(1,N)>0.5;
   ip2 = rand(1,N)>0.5;
   s = ip*2-1; % 使用比特流1生成bpsk星座图
   s2 = ip2*2-1; % 使用比特流2生成bpsk星座图
...
for ii = 1:length(Eb_N0_dB)
   ...
   ip = rand(1,N)>0.5;
   ip2 = rand(1,N)>0.5;
   s = ip*2-1; % 使用比特流1生成qam星座图
   s2 = ip2*2-1; % 使用比特流2生成qam星座图
   s = s./maxlen; % 能量归一化，只改动噪声功率
   s2 = s2./maxlen; % 能量归一化，只改动噪声功率
   n = 1/sqrt(2)*(randn(nRx,nRx,m) + 1j*randn(nRx,nRx,m)); % 高斯白噪声
   h = 1/sqrt(2)*(randn(nRx,nTx,m) + 1j*randn(nRx,nTx,m)); % 瑞利信道
