...
for ii = 1:length(Eb_N0_dB)
   ...
   ip = rand(1,N)>0.5;
   ip2 = rand(1,N)>0.5;
   s = qam16_modulation(ip); % 使用比特流1生成qam星座图
   s2 = qam16_modulation(ip2); % 使用比特流2生成qam星座图
...
for ii = 1:length(Eb_N0_dB)
   ...
   for k=1:m
       ...
       [allocatedp,water]=waterfill2(1,bottomlevel);% 一次waterfill算法
   ...
   for r=1:nRx
       for t=1:nTx
            sr(t,r,:)=ss(r,:).*sqrt(allocatedpwr(t,:)); % 动态功率分配
       end
   end
   %生成两根天线的发送序列 成为一个2*2*m的矩阵
